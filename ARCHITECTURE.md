...
> ⚠️ **Документ частично устарел (проверено 09.07.2026).** Подход Supabase-first
> подтвердился, но детали разошлись с кодом: актуальная схема — в
> `supabase/0NN_*.sql` (начиная с 026 — enum `user_role` без ролей
> `chairman`/`department_head`, `profiles.id = auth.uid()`); сводное состояние —
> `supabase/demo_full_setup.sql`. Ролевую матрицу из раздела 5 в коде не
> реализовывали. Статус функционала — см. блок «Статус реализации» в `PRD.md`.
