      ...
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={headerStyle}>
          {member.avatar_url ? (
            ...
            <div style={{ ...avatarStyle, background: "#3557d6", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700 }}>
              {getInitials(displayName)}
            </div>
          )}
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: "#1a1f2b" }}>{displayName}</div>
            <div style={{ fontSize: 14, color: "#6b7384", marginTop: 2 }}>{t(`roles.${member.role}`)}</div>
        ...
        {loading ? (
          <div style={{ padding: 24, textAlign: "center", color: "#9ba3b4" }}>{t("common.loading")}</div>
        ) : (
          <div style={{ maxHeight: "50vh", overflowY: "auto", padding: "0 24px 24px" }}>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {details.education_entries.map((entry, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ padding: "8px 12px", border: "1px solid #f5f7fa", borderRadius: 10, background: "#f5f7fa" }}>
                        {degree && <div style={{ fontSize: 13, fontWeight: 600, color: "#1a1f2b" }}>{degree}</div>}
                        {specialty && <div style={{ fontSize: 12, color: "#2a3040", marginTop: 1 }}>{specialty}</div>}
                        {institution && <div style={{ fontSize: 12, color: "#6b7384", marginTop: 2 }}>{institution}</div>}
                        {years && <div style={{ fontSize: 11, color: "#9ba3b4", marginTop: 2 }}>{years}</div>}
                      </div>
                    );
                  })}
                </div>
            ...
            {!position && !company && !bio && !education && !experience && (
              <div style={{ padding: "16px 0", color: "#9ba3b4", fontSize: 14, fontStyle: "italic" }}>
                {t("profile.noData")}
              </div>
            )}
          </div>
...
function InfoRow({ label, value }: { icon: string; label: string; value: string }) {
  return (
    <div style={{ display: "flex", gap: 8, marginTop: 10, fontSize: 14 }}>
      <span style={{ color: "#6b7384", minWidth: 100, flexShrink: 0 }}>{label}:</span>
      <span style={{ color: "#1a1f2b" }}>{value}</span>
    </div>
  );
}
...
function InfoBlock({ title, text }: { title: string; text: string }) {
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ fontSize: 14, color: "#2a3040", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{text}</div>
    </div>
  );
}
...
const closeBtnStyle: React.CSSProperties = {
  ...
  fontSize: 24, cursor: "pointer", color: "#9ba3b4", zIndex: 1,
};
const headerStyle: React.CSSProperties = {
  display: "flex", gap: 20, alignItems: "flex-start", padding: 24, borderBottom: "1px solid #f5f7fa",
};
...
const statusBadgeStyle: React.CSSProperties = {
  ...
  fontWeight: 500, background: "#f5f7fa", color: "#2a3040", marginTop: 6,
};
const blockTitleStyle: React.CSSProperties = {
  fontSize: 13, fontWeight: 600, color: "#6b7384", marginBottom: 6, };
const actionsStyle: React.CSSProperties = {
  display: "flex", gap: 8, padding: "16px 24px", borderTop: "1px solid #f5f7fa",
};
const msgBtnStyle: React.CSSProperties = {
  display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", fontSize: 14,
  fontWeight: 500, borderRadius: 10, border: "1px solid #cfd5df", background: "#fff",
  color: "#2a3040", cursor: "pointer",
};
const profileBtnStyle: React.CSSProperties = {
  padding: "8px 16px", fontSize: 14, fontWeight: 500, borderRadius: 10, border: "none",
  background: "#3557d6", color: "#fff", cursor: "pointer",
};
