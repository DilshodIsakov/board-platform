...
export async function updateMyProfile(updates: { full_name?: string; full_name_en?: string; full_name_uz?: string; email_notifications_enabled?: boolean }): Promise<boolean> {
  const { data: { session } } = await supabase.auth.getSession();
