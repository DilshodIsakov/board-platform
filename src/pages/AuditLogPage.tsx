...
function getActionColor(action: string): { bg: string; text: string } {
  if (action.includes("create") || action === "login" || action === "file_upload") return { bg: "#e7f6ec", text: "#1b6b3a" };
  if (action.includes("delete") || action === "logout" || action === "login_failed") return { bg: "#fdeaea", text: "#a12b2b" };
  if (action.includes("update") || action.includes("change")) return { bg: "#d3dbf7", text: "#2c48b8" };
  if (action.includes("vote") || action.includes("sign")) return { bg: "#f5f7fa", text: "#6B21A8" };
  if (action.includes("download") || action.includes("view")) return { bg: "#fff5dd", text: "#854D0E" };
  return { bg: "#f5f7fa", text: "#2a3040" };
}

function getStatusColor(status: string): { bg: string; text: string } {
  if (status === "success") return { bg: "#e7f6ec", text: "#1b6b3a" };
  if (status === "failed") return { bg: "#fdeaea", text: "#a12b2b" };
  return { bg: "#f5f7fa", text: "#2a3040" };
}
...
export default function AuditLogPage({ profile }: Props) {
  ...
  if (profile?.role !== "admin" && profile?.role !== "corp_secretary") {
    return <div style={{ padding: 40, textAlign: "center", color: "#9ba3b4" }}>{t("common.accessDenied") || "Access denied"}</div>;
  }
  ...
  return (
    <div style={{ padding: "32px 40px", width: "100%", boxSizing: "border-box" as const }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 700, color: "#1a1f2b", margin: 0 }}>{t("auditLog.title")}</h1>
          <p style={{ fontSize: 14, color: "#6b7384", margin: "6px 0 0" }}>{t("auditLog.subtitle")}</p>
        </div>
        <button onClick={handleExportCSV} disabled={exporting} style={{
          display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10,
          background: "#1a1f2b", color: "#FFF", border: "none", cursor: "pointer", fontSize: 14,
      ...
      {/* Filters */}
      <div style={{ background: "#FFF", borderRadius: 14, boxShadow: "var(--shadow-card)", border: "1px solid #e3e7ee", padding: "20px 24px", marginBottom: 20 }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, alignItems: "flex-end" }}>
          ...
          <div style={{ flex: "1 1 200px", minWidth: 160 }}>
            ...
            <div style={{ position: "relative" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9ba3b4" strokeWidth="2" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }}>
          ...
          {(search || userId || actionType || entityType || userRole || status || dateFrom || dateTo) && (
            <button onClick={() => { setSearch(""); setUserId(""); setActionType(""); setEntityType(""); setUserRole(""); setStatus(""); setDateFrom(""); setDateTo(""); }}
              style={{ padding: "8px 16px", borderRadius: 10, border: "1px solid #e3e7ee", background: "#FFF", color: "#6b7384", cursor: "pointer", fontSize: 13, fontWeight: 500, alignSelf: "flex-end", marginBottom: 1 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, padding: "0 4px" }}>
        <span style={{ fontSize: 13, color: "#6b7384" }}>{t("auditLog.totalRecords", { count: totalCount })}</span>
        <span style={{ fontSize: 13, color: "#9ba3b4" }}>{t("auditLog.pageInfo", { page, total: totalPages })}</span>
      </div>

      {/* Table */}
      <div style={{ background: "#FFF", borderRadius: 10, border: "1px solid #e3e7ee", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "#9ba3b4" }}>{t("common.loading")}</td></tr>
              ) : logs.length === 0 ? (
                <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "#9ba3b4" }}>{t("auditLog.noRecords")}</td></tr>
              ) : logs.map((log) => {
                const ac = getActionColor(log.action_type);
                const sc = getStatusColor(log.status);
                return (
                  <tr key={log.id} style={{ borderBottom: "1px solid #f5f7fa", cursor: "pointer", transition: "background 0.1s" }}
                    onMouseEnter={(e) => (e.currentTarget.style.background = "#f5f7fa")}
                    onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                    onClick={() => setSelectedLog(log)}>
                    <td style={tdStyle}><span style={{ whiteSpace: "nowrap", fontSize: 13, color: "#2a3040" }}>{fmt(log.created_at)}</span></td>
                    <td style={tdStyle}>
                      <div style={{ fontWeight: 500, color: "#1a1f2b" }}>{log.user_name || "—"}</div>
                      <div style={{ fontSize: 12, color: "#9ba3b4" }}>{log.user_email || ""}</div>
                    </td>
                    <td style={tdStyle}><span style={{ fontSize: 12, color: "#6b7384" }}>{t(`roles.${log.user_role}`, log.user_role || "")}</span></td>
                    <td style={tdStyle}>
                      <span style={{ display: "inline-block", padding: "3px 10px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: ac.bg, color: ac.text }}>
                        {t(`auditLog.actions.${log.action_type}`, log.action_label || log.action_type)}
                      </span>
                    </td>
                    <td style={tdStyle}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9ba3b4" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={getEntityIcon(log.entity_type)} /></svg>
                        <span style={{ fontSize: 13, color: "#6b7384" }}>{log.entity_type ? t(`auditLog.entities.${log.entity_type}`, log.entity_type) : "—"}</span>
                      </div>
                    </td>
                    <td style={{ ...tdStyle, maxWidth: 220 }}>
                      <span style={{ fontSize: 13, color: "#2a3040", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", display: "block" }}>{log.entity_title || "—"}</span>
                    </td>
                    <td style={tdStyle}>
                      <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 8, fontSize: 11, fontWeight: 600, background: sc.bg, color: sc.text }}>
                        {log.status === "success" ? t("auditLog.statusSuccess") : t("auditLog.statusFailed")}
                      </span>
                    </td>
                    <td style={tdStyle}>
                      <button onClick={(e) => { e.stopPropagation(); setSelectedLog(log); }}
                        style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid #e3e7ee", background: "#FFF", color: "#6b7384", cursor: "pointer", fontSize: 12 }}>
                        {t("auditLog.details")}
                      </button>
                    </td>
                  </tr>
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: "16px 24px", borderTop: "1px solid #f5f7fa" }}>
            <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1} style={pgBtn(page <= 1)}>&larr;</button>
            {genPages(page, totalPages).map((p, i) =>
              p === "..." ? <span key={`d${i}`} style={{ padding: "4px 2px", color: "#9ba3b4" }}>...</span> : (
                <button key={p} onClick={() => setPage(p as number)} style={{ ...pgBtn(false), background: page === p ? "#3557d6" : "#FFF", color: page === p ? "#FFF" : "#2a3040", fontWeight: page === p ? 600 : 400 }}>{p}</button>
              )
            )}
      ...
      {selectedLog && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}
          ...
          <div style={{ background: "#FFF", borderRadius: 10, width: "100%", maxWidth: 600, maxHeight: "85vh", overflow: "auto", boxShadow: "var(--shadow-overlay)", }}
            onClick={(e) => e.stopPropagation()}>
            <div style={{ padding: "24px 28px 16px", borderBottom: "1px solid #f5f7fa", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700, color: "#1a1f2b" }}>{t("auditLog.detailTitle")}</h2>
              <button onClick={() => setSelectedLog(null)} style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "#9ba3b4", fontSize: 20 }}>&times;</button>
            </div>
            <div style={{ padding: "20px 28px 28px" }}>
              <div style={{ marginBottom: 20 }}>
                {(() => { const ac = getActionColor(selectedLog.action_type); return (
                  <span style={{ display: "inline-block", padding: "6px 16px", borderRadius: 10, fontSize: 14, fontWeight: 600, background: ac.bg, color: ac.text }}>
                    {t(`auditLog.actions.${selectedLog.action_type}`, selectedLog.action_label || selectedLog.action_type)}
                  </span>
                ); })()}
                <span style={{ marginLeft: 12, fontSize: 13, color: "#9ba3b4" }}>{fmt(selectedLog.created_at)}</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "140px 1fr", gap: "12px 16px", fontSize: 14 }}>
                ...
                <DRow label={t("auditLog.col.status")} value={
                  <span style={{ padding: "2px 10px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: getStatusColor(selectedLog.status).bg, color: getStatusColor(selectedLog.status).text }}>
                ...
                {selectedLog.metadata && Object.keys(selectedLog.metadata).length > 0 && (
                  <>
                    <div style={{ gridColumn: "1 / -1", borderTop: "1px solid #f5f7fa", margin: "4px 0" }} />
                    <div style={{ gridColumn: "1 / -1", fontWeight: 600, color: "#2a3040", marginBottom: -4 }}>{t("auditLog.additionalDetails")}</div>
...
function DRow({ label, value }: { label: string; value: React.ReactNode }) {
  return <><div style={{ color: "#6b7384", fontWeight: 500 }}>{label}</div><div style={{ color: "#1a1f2b" }}>{value}</div></>;
}
...
const labelStyle: React.CSSProperties = { display: "block", fontSize: 12, fontWeight: 500, color: "#6b7384", marginBottom: 4 };
const inputStyle: React.CSSProperties = { width: "100%", padding: "8px 12px", borderRadius: 10, border: "1px solid #e3e7ee", fontSize: 13, color: "#2a3040", outline: "none", boxSizing: "border-box" };
const selectStyle: React.CSSProperties = { width: "100%", padding: "8px 12px", borderRadius: 10, border: "1px solid #e3e7ee", fontSize: 13, color: "#2a3040", background: "#FFF", cursor: "pointer", outline: "none" };
const thStyle: React.CSSProperties = { padding: "12px 16px", textAlign: "left", fontSize: 12, fontWeight: 600, color: "#6b7384", };
const tdStyle: React.CSSProperties = { padding: "12px 16px", verticalAlign: "middle" };
const pgBtn = (disabled: boolean): React.CSSProperties => ({ padding: "6px 12px", borderRadius: 10, border: "1px solid #e3e7ee", background: "#FFF", color: disabled ? "#cfd5df" : "#2a3040", cursor: disabled ? "default" : "pointer", fontSize: 13, fontWeight: 500, opacity: disabled ? 0.5 : 1 });
