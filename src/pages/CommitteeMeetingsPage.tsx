...
export default function CommitteeMeetingsPage({ profile, org }: Props) {
  ...
  if (!committee) return (
    <div style={{ color: "#d14343", padding: 32 }}>{t("common.notFound")}</div>
  );
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      <button
        ...
        style={{ background: "none", border: "none", color: "#6b7384", cursor: "pointer", fontSize: 13, marginBottom: 20, padding: 0, display: "flex", alignItems: "center", gap: 6 }}
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 16, flexWrap: "wrap", fontSize: 13, opacity: 0.9 }}>
              ...
              {myRole && (
                <span style={{ background: "rgba(255,255,255,0.22)", padding: "2px 10px", borderRadius: 8, fontWeight: 600 }}>
      ...
      {showCreate && isAdmin && (
        <div style={{
          background: "#fff", border: "1px solid #e3e7ee", borderRadius: 14, boxShadow: "var(--shadow-card)",
          padding: "20px 24px", marginBottom: 24, }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, color: "#1a1f2b", fontWeight: 600 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button
                ...
                style={{ padding: "9px 18px", fontSize: 14, background: "none", border: "1px solid #cfd5df", borderRadius: 10, cursor: "pointer", color: "#2a3040" }}
              >
                {t("common.cancel")}
              </button>
            </div>
      ...
      {/* Empty state */}
      {meetings.length === 0 ? (
        <div style={{ textAlign: "center", padding: "56px 32px", background: "#f5f7fa", borderRadius: 10, border: "1px dashed #cfd5df" }}>
          <div style={{ fontSize: 44, marginBottom: 12 }}></div>
          <div style={{ fontSize: 16, fontWeight: 600, color: "#2a3040" }}>{t("committees.noMeetings")}</div>
          {isAdmin && <div style={{ fontSize: 13, color: "#9ba3b4", marginTop: 6 }}>{t("committees.createFirst")}</div>}
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          {scheduled.length > 0 && (
            <section>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#6b7384", marginBottom: 10 }}>
          ...
          {completed.length > 0 && (
            <section>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {completed.map((m) => (
                  <MeetingCard key={m.id} meeting={m} color="#9ba3b4" committeeId={id!} navigate={navigate} t={t} isMember={isMember || isAdmin} />
                ))}
              </div>
            </section>
          )}
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: "#1a1f2b", marginBottom: 5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {title}
        </div>
        <div style={{ fontSize: 13, color: "#6b7384", display: "flex", gap: 14, flexWrap: "wrap" }}>
          <span>{dateStr}</span>
          <span>{timeStr}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <span style={{
          ...
          background: isScheduled ? "#d3dbf7" : "#f5f7fa",
          color: isScheduled ? "#2c48b8" : "#6b7384",
        }}>
          {isScheduled ? t("nsMeetings.statusScheduled") : t("nsMeetings.statusCompleted")}
        </span>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9ba3b4" strokeWidth="2.5">
          <path d="M9 5l7 7-7 7" />
        </svg>
      </div>
    </div>
