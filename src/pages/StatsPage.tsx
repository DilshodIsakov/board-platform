  ...
  const handleExport = () => {
    ...
    if (exportFormat === "csv") {
      ...
      printWin.document.write(`<html><head><meta charset="utf-8"><title>${t("stats.title")}</title>
        ...
        th,td{border:1px solid #cfd5df;padding:8px 12px;text-align:left}th{background:#f5f7fa}</style></head><body>
        <h2>${t("stats.title")}</h2>
        <p>${t("stats.totalMeetingsLabel")}: ${totalMeetings} | ${t("stats.onlineLabel")}: ${onlineMeetings} | ${t("stats.offlineLabel")}: ${offlineMeetings} | ${t("stats.decisionsLabel")}: ${decisionsCount}</p>
  ...
  return (
    <div>
      ...
      <div style={filterBarStyle}>
        <div>
          <div style={{ fontSize: 13, color: "#6b7384", marginBottom: 6, fontWeight: 500 }}>{t("stats.selectOrgan")}</div>
        ...
        <div>
          <div style={{ fontSize: 13, color: "#6b7384", marginBottom: 6, fontWeight: 500 }}>{t("stats.period")}</div>
          <select
            value={period}
        ...
        <div>
          <div style={{ fontSize: 13, color: "#6b7384", marginBottom: 6, fontWeight: 500 }}>{t("stats.exportFormat")}</div>
          <select
            value={exportFormat}
      ...
      <div style={tableCardStyle}>
        ...
        {meetings.length === 0 ? (
          <p style={{ color: "#9ba3b4", fontSize: 15 }}>{t("stats.noMeetingsForPeriod")}</p>
        ) : (
              ...
              {meetings.map((m) => {
                ...
                return (
                  <tr key={m.id} style={{ borderBottom: "1px solid #f5f7fa" }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: 500, color: "#1a1f2b" }}>{getLocalizedField(m as unknown as Record<string, unknown>, "title")}</td>
                    <td style={tdStyle}>
                    ...
                    <td style={tdStyle}>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        {isOnline ? (
                          <>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#2e9e5b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                            <span style={{ color: "#2e9e5b", fontSize: 14 }}>{t("stats.online")}</span>
                          </>
                        ) : (
                          <>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6b7384" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                              <path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                              <path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
                            <span style={{ color: "#6b7384", fontSize: 14 }}>{t("stats.offline")}</span>
                          </>
                        )}
                      </div>
                    </td>
                    <td style={{ ...tdStyle, color: "#6b7384" }}>—</td>
                    <td style={tdStyle}>
                      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3557d6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                        <span style={{ color: "#3557d6", fontSize: 14 }}>—</span>
                      </div>
                    </td>
                    <td style={{ ...tdStyle, fontWeight: 600, color: "#2a3040" }}>—</td>
                  </tr>
                );
              })}
            </tbody>
  ...
  return (
    <div style={kpiCardStyle}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        ...
        {trend === "up" && (
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2e9e5b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M23 6l-9.5 9.5-5-5L1 18" />
            <path d="M17 6h6v6" />
          </svg>
        )}
      </div>
      <div style={{ fontSize: 36, fontWeight: 700, color: "#1a1f2b", lineHeight: 1.2 }}>{value}</div>
      <div style={{ fontSize: 14, color: "#6b7384", marginTop: 4 }}>{label}</div>
    </div>
  );
}
...
function CalendarIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#3557d6" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function VideoIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#2e9e5b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
    </svg>
  );
}
...
function DocIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#d14343" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function ChartIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#6b7384" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
